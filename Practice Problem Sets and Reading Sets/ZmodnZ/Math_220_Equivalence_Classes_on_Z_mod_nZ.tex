...
\end{enumerate}
\item We say $G = G_1 \trianglerighteq G_2 \trianglerighteq \ldots \trianglerighteq G_{r+1} = \set{1}$ if at every stage, $G_{i+1}$ is maximal in $G_i$. Using normal groups $K \trianglelefteq G$, we can form quotient groups $G/K$ where its order is $[G:K]$ and its elements are cosets of $K$ with multiplication being defined as $g_1K * g_2 K = (g_1\cdot g_2) K$ where $\cdot$ is the multiplication in $G$. \begin{enumerate}
